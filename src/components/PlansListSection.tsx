...
function PlanListRowDetails({ plan, userId, t }: { plan: PlanRow; userId: string; t: Messages }) {
  return (
    <>
      <div className="flex min-w-0 flex-wrap items-center gap-2 pt-0.5 sm:flex-nowrap sm:gap-2">
        <span
        ...
        <span
          className={`inline-flex shrink-0 rounded-full px-3 py-1 text-sm font-medium ${getStatusPillClasses(plan.status)}`}
        >
...
export function PlansListSection({
  ...
  return (
    <ul className="divide-y divide-blue-100 dark:divide-zinc-700">
      {plans.map((plan) => {
        const listImageUrl =
          plan.logoAttachmentId != null
            ? `/api/plans/${plan.id}/attachments/${plan.logoAttachmentId}`
            : plan.imageUrl;
        return (
        <li
          key={plan.id}
          className="relative transition hover:bg-blue-50/40 dark:hover:bg-zinc-800/50"
        >
          <Link
            href={`/plans/${plan.id}`}
            prefetch={false}
            className={
              listImageUrl
                ? "flex min-w-0 flex-col px-6 py-4 text-inherit no-underline md:flex-row md:items-start md:p-0"
                : "flex min-w-0 flex-col px-6 py-4 text-inherit no-underline"
            }
          >
            {listImageUrl ? (
              <div className="flex min-w-0 items-start gap-3 md:contents">
                <div className="shrink-0 md:pl-6 md:pr-3 md:py-4">
                  {/* Same-origin authenticated URL; <img> sizes reliably in the two-column layout */}
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={listImageUrl}
                    alt=""
                    className="h-14 w-14 rounded-lg border border-border object-contain md:h-[4.5rem] md:w-[4.5rem]"
                  />
                </div>
                <div className="flex min-w-0 flex-1 flex-col gap-2 md:min-w-0 md:flex-1 md:py-4 md:pl-0 md:pr-6">
                  <PlanListRowDetails plan={plan} userId={userId} t={t} />
                </div>
              </div>
            ) : (
