...
- When the user provides a set of tasks in JSON form (e.g. exported plan/task list), treat this as a **bulk task → PR pipeline**:
  ...
  3. **Analysis docs for deferred tasks:** For any task that is **documented for later** (not implemented in the bulk run), create a **dedicated analysis doc** in `docs/` in the same format as existing ones (e.g. `docs/TECH-0026-permanent-website.md`). Each doc must include: **Status** (e.g. “Not implemented — implementation notes for future work”), **Goal** (one sentence), **What is needed** (numbered sections with options/tables where useful), and a **Summary checklist**. Use the filename pattern `docs/TECH-<ID>-<kebab-description>.md`. Add the new doc to `docs/FUTURE_WORK.md` and, if using a bulk index (e.g. `docs/BULK-PLAN-2026-03-REMAINING.md`), link the task row to the analysis doc. This ensures every deferred task from a bulk run has a single place for implementation notes and checklists.
