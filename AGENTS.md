...
- When the user provides a set of tasks in JSON form (e.g. exported plan/task list), treat this as a **bulk task → PR pipeline**:
  ...
  2. **Implement or document:** For each task, either implement it in code or add/update existing future-work documentation when the scope is too large for a single PR. **Prefer implementing small/quick tasks** (e.g. layout-only, single-file CSS, one component change, no schema or new APIs) rather than deferring them. Only defer and write an analysis doc when the task is genuinely large (multi-area, schema change, external integration, or unclear scope). When in doubt, attempt implementation first; document only if the work clearly exceeds one PR or requires design/exploration.
  3. **Analysis docs for deferred tasks:** For any task that is **documented for later** (not implemented in the bulk run), create a **dedicated analysis doc** in `docs/` in the same format as existing ones (e.g. `docs/TECH-0026-permanent-website.md`). Each doc must include: **Status** (e.g. “Not implemented — implementation notes for future work”), **Goal** (one sentence), **Why deferred** (one line: e.g. “Deferred in bulk run due to run scope” or “Scope too large for one PR”; include **Estimated effort** when obvious — e.g. “Estimated effort: small (layout only)” or “Estimated effort: large (schema + API)” so future runs can triage), **What is needed** (numbered sections with options/tables where useful), and a **Summary checklist**. Use the filename pattern `docs/TECH-<ID>-<kebab-description>.md`. Add the new doc to `docs/FUTURE_WORK.md` and, if using a bulk index (e.g. `docs/BULK-PLAN-2026-03-REMAINING.md`), link the task row to the analysis doc. This ensures every deferred task has a single place for implementation notes and makes it clear why it was deferred and whether it is a quick win.
