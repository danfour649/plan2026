...
- When the user provides a set of tasks in JSON form (e.g. exported plan/task list), treat this as a **bulk task → PR pipeline**:
  ...
  6. **Bulk-run quality gate:** Before considering a bulk-task implementation done (and before pushing or opening the PR), run **`npm run lint`**, **`npm run typecheck`**, and **`npm run build:next`** (or `npm run prepush`) and fix any failures—unless the user explicitly asked to skip checks for speed. Catching lint and type errors on the branch avoids follow-up incident PRs. If the user has asked to skip checks, omit this step.
